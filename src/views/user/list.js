...
const UserList = () => {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/user/getuserall`, {
        method: 'POST',
  ...
  const handleModifyClick = (UserID) => {
    navigate(`/users/modify?UserID=${UserID}`);
  };
            ...
            <tbody>
              {users.map((user, index) => (
                <tr key={user.UserID}>
                  ...
                  <td>
                    <CIcon onClick={() => handleModifyClick(user.UserID)} icon={cilPencil} size="lg" className="edit-icon" />
                    <CIcon onClick={() => handleDeleteClick(user.UserID)} icon={cilTrash} size="lg" className="trash-icon" />
                  </td>
                </tr>
              ))}
            </tbody>
